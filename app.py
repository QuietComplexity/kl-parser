...
# --- PRZYCISK RESETU (Wyczyść wszystko) ---
...
with col_in:
    ...
    # Pole tekstowe bez zapamiętywania stanu (czyści się przy rerun)
    input_text = st.text_area("Wklej treść razem z metadanymi:", height=550, key="main_text")
...
if input_text:
    ...
    for l in content_lines:
        if "[" in l and "]" in l:
            ...
            # Wybór szerokości
            width = w_vertical if any(k in tag_raw for k in ["pion", "v", "sq", "kwadrat"]) else w_horizontal
    ...
    with col_out:
        st.info("Wklej tekst po lewej stronie, aby rozpocząć.")