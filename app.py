...
if input_text:
    ...
    for l in content_lines:
        ...
        # 1. PRZYPISY NUMEROWANE (np. [1]) - musimy to sprawdzić ZANIM sprawdzimy obrazki
        if re.match(r'^\[\d+\]', l_strip):
            html_body.append(f'<span style="font-weight: 400;">{l_strip}</span>')

        # 2. OBRAZKI (Tylko jeśli w środku nie ma samej cyfry)
        elif "[" in l_strip and "]" in l_strip and not re.search(r'^\[\d+\]$', l_strip):
            tag_match = re.search(r'\[(.*?)\]', l_strip)
            if tag_match:
                tag_raw = tag_match.group(1).lower()
        ...
        # 3. NAGŁÓWKI SEKCJI (Przypisy / Książka)
